...
const Lista = () => {
    ...
    function getEspecialidad() {
        ...
        get(query(ref(db, 'especialidades/'), orderByKey())).then((snapshot) => {
            if (snapshot.exists()) {
                ...
                especialidades.push({ id: null, name: 'Todas' });
            } else {
    ...
    function filtrarEspecialidad(value) {
        ...
        setDataFiltrada(data.filter(e => {
            if (e.especialidad == value) return e;
        }));
        dataFiltrada.filter(e => {
            if (e.especialidad == value) return e;
        });
        /*if (value != null) {
            data.forEach((doctor, i) => {
                console.log(doctor.especialidad + ' ' + value.id);
                if (i == 0 && dataFiltrada.length > 0) setDataFiltrada([]);
                if (doctor.especialidad == value.id) dataFiltrada.push(doctor);
            })
        } else {
                ...
                <br></br>
                    ...
                    <table class="w-10/12 text-sm text-left border ">
                        ...
                        <tbody>
                            ...
                            {dataFiltrada.map((doctorSnapshot, i) => (
